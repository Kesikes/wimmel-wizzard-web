  ...
  const SCENARIOS = [
    ...
    },
    {
      ...
      async run(T){
        ...
        // Beide Augen bleiben voll sichtbar.
        const threeQInstr = "Redraw this exact character in THREE QUARTER VIEW: the whole body and head are turned slightly to the right. Keep the exact same hair style and color, headwear, clothing and all identifying details as the reference. The head is a round shape with no ears, no mouth and no visible neck. The entire facial feature group – both eyes AND the nose together, as one unit – is shifted slightly to the right of the vertical center of the face (not spread apart, not centered). Both eyes stay fully visible as small dots, positioned close together and shifted to the right side of the face; the nose is positioned right below the midpoint between the two eyes and moves right together with them. Draw the nose as ONE perfectly straight vertical line segment, like the keyboard character \"|\" – completely straight from top to bottom, with NO curve, NO hook, NO bend, NO foot or serif at the bottom end, NOT shaped like a check mark, a hook, a \"J\", or an upside-down \"L\". Match the exact illustration style of the reference image: thick black marker outline, flat colors with soft cel-shading exactly as shown, rosy cheeks. Full body, standing, plain white background.";
