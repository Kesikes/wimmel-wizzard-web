/* ==========================================================================
   Wimmel Wizard v3 — Screen „Dashboard" (Schritt 2)
   Texte/Werte wörtlich aus referenz/App-Flow-v4-OatlyWimmel.dc.html.
   Hinweis: Die Fortschritts-Bruchzahlen (2/5 Charaktere, 1/5 Wimmelbilder)
   sind 1:1 aus der Referenz übernommen und bewusst nicht an die 6-köpfige
   Personen-Liste im Charakterblatt-Screen angeglichen — die Referenz selbst
   verwendet für Dashboard und Charakterblatt unterschiedliche Zählweisen.
   ========================================================================== */
...
Screens.dashboard = {
  render(root) {
    ...
    wrap.appendChild(h("h1", { class: "h-black", style: { margin: "26px 0 3px", fontSize: "30px", lineHeight: ".9", letterSpacing: "-.04em" } }, [h("span", {}, "Eure"), h("br"), h("span", {}, "Bausteine.")]));
    ...
    cards.appendChild(buildBlockCard({
      badge: "hier starten", badgeBg: "var(--red)",
      ring: 40, ringLabel: "2/5", shadow: "var(--red)",
      title: "Charaktere",
      body: "Mia und Papa sind schon gezeichnet. Oma fehlt noch.",
      onClick: () => Router.goScreen("charakter")
    }));

    cards.appendChild(buildBlockCard({
      ring: 20, ringLabel: "1/5", shadow: "var(--ink)",
      title: "Wimmelbilder",
      body: "„Nachmittag am See“ ist fertig. Nächste Szene wartet.",
    ...
    root.appendChild(buildDesktopDashboard());
  }
};

function buildDesktopDashboard() {
  ...
  stand.appendChild(standRow("Charaktere", "3 von 5"));
  stand.appendChild(standRow("Wimmelbilder", "1 von 5"));
  stand.appendChild(standRow("Produkt", "Mini-Buch · 49 €"));
  ...
  charCard.appendChild(h("span", { style: { display: "block", margin: "10px 0 16px", fontSize: "15px", lineHeight: "1.5" } }, "Mia, Papa und Oma Rosi sind gezeichnet. Bruno, Mama und der Hund fehlen noch."));
  const peopleRow = h("span", { style: { display: "flex", gap: "8px" } });
  PEOPLE_GRID.forEach((p) => {
    peopleRow.appendChild(h("span", { style: { display: "flex", alignItems: "center", justifyContent: "center", width: "40px", height: "40px", fontFamily: "'Archivo Black',sans-serif", fontSize: "15px", border: p.done ? "3px solid var(--ink)" : "3px dashed rgba(26,26,24,.45)", background: p.done ? "var(--blue)" : "transparent", color: p.done ? "var(--ink)" : "rgba(26,26,24,.5)" } }, p.mark));
  });
  ...
  imgCard.appendChild(h("span", { style: { display: "block", margin: "10px 0 16px", fontSize: "15px", lineHeight: "1.5" } }, "„Nachmittag am See“ ist fertig. Die nächste Szene wartet auf eine Geschichte."));
  const thumbRow = h("span", { style: { display: "flex", gap: "8px" } });
  thumbRow.appendChild(h("img", { src: "assets/example_gardasee.png", alt: "", style: { width: "88px", border: "3px solid var(--ink)" } }));
