...
module.exports = async (req, res) => {
  ...
  if (!FAL_KEY) {
    res.status(500).json({ error: "Server-Fehler: FAL_KEY ist im Vercel-Projekt nicht gesetzt.", vorAufruf: true });
    return;
  }
  ...
  if (body.mode === "verify") {
    ...
    if (!verifyImageUrls.length || !verifyPrompt) {
      res.status(400).json({ error: "Verify: imageUrls und verifyPrompt erforderlich.", vorAufruf: true });
      return;
    }
  ...
  if (body.mode === "style_pass") {
    ...
    if (!sourceUrl || !stylePrompt) {
      res.status(400).json({ error: "style_pass: imageUrl und prompt erforderlich.", vorAufruf: true });
      return;
    }
    try {
  ...
  // Die Antwort sagt jetzt AUSDRUECKLICH, dass der bezahlte Dienst noch gar nicht gerufen wurde
  // (vorAufruf -- das Feld heisst bewusst nicht "vorFal", es gilt auch fuer Anthropic und OpenAI)
  // und nennt
  ...
  if (imageUrl && imageUrl.startsWith("data:") && imageUrl.length > 4_000_000) {
    res.status(400).json({ error: "Foto ist zu groß.", vorAufruf: true });
    return;
  }
  ...
  if (!imageUrl && !prompt.startsWith("wmlstil")) {
    res.status(400).json({ error: "Prompt-Format ungültig.", vorAufruf: true });
    return;
  }
